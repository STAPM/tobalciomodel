...
These elasticities are coded into `tobalciomodel` as package data in `tobalciomodel::elasticities_alc` for alcohol-related elasticities and `tobalciomodel::elasticities__alc_tob` tobacco:

```{r elasticities_alc_names}
tobalciomodel::elasticities_alc
```

```{r elasticities_tob_names}
tobalciomodel::elasticities_tob
```

## Employment

Employment data are not included with IO tables, so in order to model the impacts of policy on employment this information must be obtained externally and matched to the sectors in the 106-sector IO table. 

### Obtaining and Cleaning Employment Data from the Labour Force Survey

Employment data is obtained from the quarterly Labour Force Survey (LFS) and processed using the `lfsclean` version 0.6.2 package. Employment data coded according to SIC-2007 industries has been available since 2008. Annual employment figures by 4-digit industry to then map to the 106 industries used in the FAI input-output table. Employment is measured as total number of full-time equivalents (FTEs), with a part-time employee counted as 0.5 FTE. Employment can also be measured in hours, using the total hours usually worked per week.   
...
This produces employment figures for the 106 sectors in the FAI IO table and is found in the dataset `tobalciomodel::employment`, sorted in order of sector in the FAI tables and so can be easily matched to `tobalciomodel::iotable` on a one-to-one basis. Both headcount and FTE measures of employment are produced for each year 2010-2020 inclusive.  
...
In order to compare estimated effects to the size of the economy as a whole, the package includes a yearly time series of macroeconomic variables. [Annual figures for Gross Value Added are obtained from the Office for National Statistics](https://www.ons.gov.uk/economy/grossvalueaddedgva/timeseries/abml/pn2) valued in £m in a time series from 1955-2019. GDP and GVA figures in £millions [also available from the ONS](https://www.ons.gov.uk/economy/grossdomesticproductgdp/datasets/uksecondestimateofgdpdatatables). In order to adjust figures for inflation, [the CPIH index 1988-2019 is also obtained from the ONS](https://www.ons.gov.uk/economy/inflationandpriceindices/timeseries/l522/mm23). Aggregate employment is added to the macroeconomic variables dataset by summing up the sector-level employment created from the LFS described above. 
